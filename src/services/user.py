...
from src.gateways.postgresql.models.user import (
    FriendORM,
    UserORM,
)
...
class ORMUserService(IUserService, IFriendService):
    ...
    async def get_by_telegram(self, telegram_id: str) -> User | None:
        ...
        async with self.database.get_ro_session() as session:
    ...
    async def get_friends(self, user_oid: UUID) -> list[User]:
        ...
        async with self.database.get_ro_session() as session:
            friends_dto = await session.execute(stmt)
        return [friend_dto.to_entity() for friend_dto, *_ in friends_dto]
